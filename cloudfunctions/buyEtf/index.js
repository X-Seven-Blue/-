...
exports.main = async (event = {}) => {
  try {
    ...
    if (!Number.isInteger(amount) || amount <= 0) return fail('INVALID_AMOUNT', 'amount must be a positive integer')
    ...
    if (beforeBalance < amount) return fail('INSUFFICIENT_CAN', 'can balance is insufficient')

    const afterBalance = beforeBalance - amount
    const balanceKey = getBalanceKey(assets)
    const price = Number(quote.price)
    const shares = round(amount / price, 6)
    ...
    await db.collection('user_assets').doc(assets._id).update({ data: { [balanceKey]: _.inc(-amount), updatedAt: now() } })
    await db.collection('can_ledgers').add({
      data: {
        ...
        relatedId: tradeId,
        businessId: tradeId,
        remark: 'ETF learning buy',
        createdAt: now()
      }
    })
    await db.collection('etf_trades').add({ data: trade })

    const position = await updateOrCreatePosition({
      openid: ctx.openid,
      quote,
      sharesDelta: shares,
      costDelta: amount
    ...
    console.error('[buyEtf] failed:', error)
    return fail('SERVER_ERROR', error.message || 'buyEtf failed')
  }
}
