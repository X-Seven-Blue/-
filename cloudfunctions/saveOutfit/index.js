...
const BUILTIN_OUTFITS = ['default', 'ribbon_red', 'hat_blue', 'bell_gold']
...
exports.main = async (event = {}) => {
  try {
    ...
    const outfitId = event.outfitId
    if (typeof outfitId !== 'string' || !/^[a-zA-Z0-9_-]{1,40}$/.test(outfitId)) return fail('INVALID_PARAM', 'outfitId 必须是合法字符串')
    if (!BUILTIN_OUTFITS.includes(outfitId)) return fail('INVALID_PARAM', '不支持的装扮')

    const assets = await findAssets(openid, userId)
    const assetOwned = assets && Array.isArray(assets.ownedOutfits) ? assets.ownedOutfits : null
    const ownedOutfits = assetOwned || BUILTIN_OUTFITS
    if (!ownedOutfits.includes(outfitId)) return fail('INVALID_PARAM', '用户未拥有该装扮')

    const cat = await findCurrentCat(openid, userId)
    if (!cat) return fail('CAT_NOT_FOUND', '当前小猫不存在')
    const outfit = { current: outfitId, owned: ownedOutfits }
    await db.collection('cats').doc(cat._id).update({ data: { outfit, outfitId, updatedAt: db.serverDate() } })

    return ok({ catId: cat.catId || cat._id, currentOutfit: outfitId, ownedOutfits })
  } catch (error) {
    console.error('[saveOutfit] failed:', error)
    return fail('INTERNAL_ERROR', error.message || '保存装扮失败')
  }
}
